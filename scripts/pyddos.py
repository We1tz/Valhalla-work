...
def ddos(dom, tout, pak, prt, slp, spip, fip, tip):
    title = '''
	
		  _ \        __ \                ___|           _)       |   
		 |   | |   | |   |   _ \   __| \___ \   __|  __| | __ \  __|  
		 ___/  |   | |   |  (   |\__ \       | (    |    | |   | |   
		_|    \__, |____/  \___/ ____/ _____/ \___|_|   _| .__/ \__|  
                       ____/                                       _|    
    ...
    class Synflood(Thread):
        ...
        def main(tout, dom, pak, prt, slp, spip, fip, tip):
                ...
                ###options.add_argment('-p', metavar='<int>', default=80,
                ###                   # help='Specify port target (default = 80)' + colored(
                ###                    #    ' |Only required with pyslow attack|', 'red'))
                ...
                if tip == 'Synflood':
                    ...
                    while 1:
                        try:
                            ...
                            sys.exit(cprint('[-] Canceled by user', 'red'))
                if not (tip == 'Synflood') and not (tip == 'Request') and not (tip == 'Pyslow'):
                    parser.print_help()
                    print
                    sys.exit(cprint('[-] You must choose attack type', 'red'))

        main(tout, dom, pak, prt, slp, spip, fip, tip)
    pass