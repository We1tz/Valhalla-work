...
def boynextdork(dorka):
    ...
    def dorking(domain):
        ...
        f.close()
    domain = dorka
    if domain and re.match("^((?!-))(xn--)?[a-z0-9][a-z0-9-_]{0,61}[a-z0-9]{0,1}\.(xn--)?([a-z0-9\-]{1,61}|[a-z0-9-]{1,30}\.[a-z]{2,})$", domain):
        try:
            dorking(domain)
            print(f"\n[+] Success. Please check the {domain} directory.\n")
        except:
            print(f"\n[-] Directory '{domain}' is already present.\n")
    pass
